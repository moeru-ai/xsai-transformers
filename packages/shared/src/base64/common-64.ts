...
export const rAlphabet: Record<Base64Format, Uint8Array> = {
  Base64: Uint8Array.from(Array.from({ length: 128 }).fill(64)), // alphabet.Base64.length
 ...
 * ```
 */
export const calcSizeBase64 = (originalSize: number): number => {
...
const getByte = (char: number, alphabet: Uint8Array): number => {
